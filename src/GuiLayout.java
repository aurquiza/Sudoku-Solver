	...
	// instance variables to help initialize all necessary aspects of the gui
 	...
 	public GuiLayout(List<String> input)
 	{
		...
		//will added to the master panel
 		...
 		CandidatePanel = new JPanel(new GridLayout());
 		candidateDisplay = new JLabel("candidate list", JLabel.CENTER);
 		CandidatePanel.add(candidateDisplay);
		...
		// add sub-panels to the master panel
 		...
 		masterPanel.add(CandidatePanel, BorderLayout.SOUTH);
 	...
 	// sets the candidate list to display on the gui
 	public void setCandidateList(String[] l)
 	{
 		candidateDisplay.setText("Candidates: " + l[0] + " " + l[1] + " " + l[2] + " " + l[3] +
 								 " " + l[4] + " " + l[5] + " " + l[6] + " " + l[7] +
 								 " " + l[8]);
 	}
 	...
 	public  void displayLoadedBoard(int a, int b, int c)
 	{
 		...
 		{
 			System.err.println("INVALID INPUT: " + a + " " + b + " : " + c);
 		...
 		int xPos = convertXnY(a);
 		int yPos = convertXnY(b);
 		...
 		Candidate cand = new Candidate(grid);
 		cand.removeCandidate(Integer.toString(c), twoDArray[xPos][yPos].getCellSection(), twoDArray[xPos][yPos]);
 		cand.deleteAllCandidates(twoDArray[xPos][yPos].getCandidates());
 	}
